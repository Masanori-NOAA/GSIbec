...
! Declare local variables
  ...
  if (header_fix%iextra > 0) then
     call nc_diag_read_get_var('Press_Max_Weight_Function', Press_Max_Weight_Function)
  endif
  if (header_fix%angord > 0) then
     call nc_diag_read_get_var('BC_angord ', BC_angord )
  end if
  if (header_fix%ijacob > 0) then
     call nc_diag_read_get_var('Observation_Operator_Jacobian', Observation_Operator_Jacobian)
  endif
  cdatum = 1
