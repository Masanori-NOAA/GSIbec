...
! !USES:
  ...
  use fgrid2agrid_mod, only: nord_f2a,final_fgrid2agrid,set_fgrid2agrid
...
!                        avoid "use gridmod" in m_mpimod.F90, which causes compiler conflict, since
...
!            add use of guess file later for regional mode.
  ...
  namelist/setup/gencode,factqmin,factqmax,superfact,limitqobs,clip_supersaturation, &
       factql,factqi,factqr,factqs,factqg, &     
       factv,factl,factp,factg,factw10m,facthowv,factcldch,R_option,&
       ...
       use_compress,nsig_ext,gpstop,commgpstop, commgpserrinf, &
       perturb_obs,perturb_fact,oberror_tune, &
       berror_stats,tcp_posmatch,tcp_box, &
       lobsdiagsave, lobsdiag_forenkf, &
       l4dvar,lbicg,lsqrtb,lcongrad,lbfgsmin,ltlint,nhr_obsbin,nhr_subwin,&
       mPES_observer,&
       use_fv3_aero,&
       nwrvecs,iorthomax,ladtest,ladtest_obs, lgrtest,lobskeep,jsiga,ltcost, &
       idmodel,iwrtinc,lwrite4danl,nhr_anal,jiterstart,jiterend,lobserver,lanczosave, &
       print_diag_pcg,tsensible,lread_obs_save,lread_obs_skip, &
       use_gfs_ozone,check_gfs_ozone_date,regional_ozone,lwrite_predterms,&
       lwrite_peakwt,use_gfs_nemsio,use_gfs_ncio,sfcnst_comb,liauon,l4densvar,ens_nstarthr,&
       step_start,diag_precon,lrun_subdirs,&
       use_sp_eqspace,lnested_loops,thin4d,use_readin_anl_sfcmask,&
       luse_obsdiag,verbose,print_obs_para,lnobalance, &
       ...
       if_model_dbz,if_model_fed,innov_use_model_fed,netcdf_diag,binary_diag,l_wcp_cwm,aircraft_recon,&
       write_fv3_incr,incvars_to_zero,incvars_zero_strat,incvars_efold,&
       lcalc_gfdl_cfrac,tau_fcst,efsoi_order,cnvw_option,l2rwthin,hurricane_radar,&
...
! Read in user specification of state and control variables
 ...
 if(mype==0) write(6,*)' at 0 in gsimod, nems_nmmb_regional = ', &
                       nems_nmmb_regional
